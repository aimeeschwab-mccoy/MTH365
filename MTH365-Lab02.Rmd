...
The R chunk below reads in the data. Make sure to change your file directory!

```{r}
pokemon <- read.csv("~/OneDrive - Creighton University/Fall 2019 Courses/MTH 365 - Intro to Data Science/Data/pokemon.csv")
```

> 1. Calculate the diference in heights pre and post evolution and save this as a new variable. Calculate the percentage of Pokémon that grew during evolution. Also visualize the distribution of change in height by species and provide a discussion of how change in height varies across species. (Include a link to any online examples you use to help.)

- _Hint_: The `mutate` function might be helpful! 

> 2. Recreate the following plot. In addition to plotting the appropriate variables, you'll need to specify a new title, __subtitle__, axis labels, legend label, and __plot theme__ to fully recreate this.

```{r, echo=FALSE, out.height = '80%'}
knitr::include_graphics("Images/2-pokemon.jpeg")
```

> 3. Use the `scale_fill_manual()` command to customize your plot. You can use hex color definitions, RGB color definitions, or the pre-defined named colors in R (http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf).

> 4. Pick two categorical variables and make a bar plot that depicts the relationship between them. Use faceting to add an additional variable. These can be variables from the original data or ones that you create. Explain what the plot tells you in 2-3 sentences.

> 5. Pick two numerical variables and make a scatterplot that depicts the relationship between them. Use faceting and color to add additional variables. These can be variables from the original data or ones that you create. Explain what the plot tells you in 2-3 sentences.

> 6. Pick a numerical and a categorical variable, and construct side-by-side box plots depicting the relationship between them. These can be variables from the original data or ones that you create. Explain what the plot tells you in 2-3 sentences.

> 7. Learn something new: violin plots! Read about them at http://ggplot2.tidyverse.org/reference/geom_violin.html, and convert your side-by-side box plots from the previous task to violin plots. What do the violin plots reveal that box plots do not? What features are apparent in the box plots but not in the violin plots?

> 8. What characteristics correspond to an evolved Pokémon with a high combat power? You do not need to come up with an exhaustive list, but you should walk through your reasoning for answering this question and include all relevant summary statistics and visualizations.